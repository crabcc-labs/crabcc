# Task 7 — Python resolver: import + class-aware scope walker
...
Task 4 split `extract.rs` into a `mod extract` directory; per-language
resolvers live under `crates/crabcc-core/src/extract/`. Task 5 added
`pub mod resolve_rust;` to `extract/mod.rs`. This task adds the Python
resolver file only — wiring `pub mod resolve_python;` into
`extract/mod.rs` is handled by a separate task and is OUT OF SCOPE here.

## What to change

### File 1 — `crates/crabcc-core/src/extract/resolve_python.rs` (NEW)
...
Then commit with this exact message:
    ...
    feat(resolve): Python scope walker — import + class-aware resolution
