# Task 6 — TS/JS resolver: ES imports + class-aware scope walker
...
`pub mod resolve_rust;` to `extract/mod.rs`. This task adds the TS/JS
resolver file only — wiring `pub mod resolve_ts;` into `extract/mod.rs`
is handled by a separate task and is OUT OF SCOPE here.

## What to change

### File 1 — `crates/crabcc-core/src/extract/resolve_ts.rs` (NEW)
...
Then commit with this exact message:
    ...
    feat(resolve): TS/JS scope walker — ES imports + class-aware resolution
