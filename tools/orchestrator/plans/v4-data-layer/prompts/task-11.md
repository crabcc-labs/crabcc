# Task 11 — KG op: importers (file-level reverse rollup)
...
`importers(target_path, max_depth)` answers: "which files transitively
depend on the symbols defined in `target_path`?". The op rolls up
symbol-level edges into a file-level view: any edge whose destination
symbol lives in the target file (or in any file reached by previous BFS
rounds) counts toward the source file's "edges-to-target" tally. BFS
expands depth-by-depth at the file granularity, capped at `max_depth`.

This is the only file-grained KG op of the four — the other three
(blast-radius, why, hot-symbols) work in symbol-id space. Importers is
intentionally coarser because the typical use is "what should I rebuild
when I touch this file?", and the file-level answer is both more
compressible and more actionable.
...
This task ONLY creates `crates/crabcc-core/src/query/importers.rs`. Do
NOT touch `crates/crabcc-core/src/query/mod.rs` — Task 8 owns the
restructure and Task 12 wires `pub mod importers;` into the module tree
when it integrates the CLI dispatcher.

## What to change

### Create `crates/crabcc-core/src/query/importers.rs`
...
Then commit with this exact message:
    ...
    feat(query): importers — file-level reverse rollup
