# Task 8 — KG op: blast_radius + restructure query.rs into query/
...
This task ALSO does the file-shape restructure that the W2b sub-wave needs:
the existing `crates/crabcc-core/src/query.rs` becomes
`crates/crabcc-core/src/query/mod.rs` (content identical), and a sibling
`blast_radius.rs` is added next to it. The `pub mod blast_radius;`
declaration goes at the top of `query/mod.rs`.

`blast_radius(root, max_depth)` answers: "if I change symbol R, which other
symbols transitively depend on it?". It is a reverse-direction BFS over
incoming edges (`WHERE dst_symbol_id = ? AND kind IN (...)`), bounded by
`max_depth`. Result is the set of affected symbol records plus a per-node
depth map (smallest depth at which we reached each affected node).

This task assumes Task 2 (Store API for v4) has landed a
`pub fn conn(&self) -> &rusqlite::Connection` accessor on `Store` so that
new query modules can issue raw SQL against the v4 edges table without
needing per-op write methods. The implementation below calls `store.conn()`.

## What to change

### 1. Delete the old single-file query.rs and create `query/mod.rs`

Read the current contents of `crates/crabcc-core/src/query.rs` and move
them verbatim into a new file at `crates/crabcc-core/src/query/mod.rs`.
Then delete `crates/crabcc-core/src/query.rs`. (Rust treats `query.rs` and
`query/mod.rs` as equivalent module roots — picking the `mod.rs` form is
required so we can add sibling files like `blast_radius.rs` next to it.)

At the very top of the new `query/mod.rs`, immediately after the existing
`use` block (before the first `pub` item), add:

```rust
pub mod blast_radius;
```

Do not modify anything else in the file. Re-exports, helpers, and tests
stay byte-identical.

### 2. Create `crates/crabcc-core/src/query/blast_radius.rs`
...
Then commit with this exact message:
    ...
    feat(query): blast_radius — reverse transitive walk; restructure query.rs → query/mod.rs
