...
impl SamplingHandler for LiteLlmSamplingHandler {
    fn handle(&self, request: SamplingRequest) -> Result<SamplingResponse, SamplingError> {
        ...
        let model = self
            .select_model(request.model_preferences.as_ref())?
            .clone();
...
pub fn build_summary_request(corpus: &str) -> SamplingRequest {
    SamplingRequest {
        ...
        system_prompt: Some(
            "You are a terse summariser. Output plain prose; no markdown headings.".into(),
        ),
        max_tokens: Some(512),
...
mod tests {
    ...
    impl SamplingObserver for CountingObserver {
        ...
        fn on_subcall_request(
            ...
            assert_eq!(
                parent_id, 42,
                "subcall must link to parent id from on_request"
            );
