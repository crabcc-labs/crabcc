...
fn emit_mythos_closed_source(name: &str) -> String {
    // Header-only constant; the .cpp exists so the module has an appfile to
...
/// Fields used (`incfiles`, `appfiles`) are taken verbatim from those files; no
...
mod tests {
    ...
    fn emit_mcconf_uses_grounded_fields() {
        ...
        assert!(m.mcconf.contains("appfiles"), "real MyThOS field");
    }
}
