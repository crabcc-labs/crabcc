...
impl SqliteBackend {
    pub fn open(path: &Path) -> Result<Self> {
        ...
        // table, so v2.1 databases (no FTS at write time) will own an empty
        // index after the first upgraded open. If drawers > 0 but FTS rows
        // == 0 the lexical path would silently return zero hits forever;
