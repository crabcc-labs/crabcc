...
mod tests {
    ...
    #[test]
    fn flag_enables_regardless_of_env() {
        assert!(enabled(true));
    }

    #[test]
    fn default_context_mentions_the_three_reminders() {
        assert!(DEFAULT_CONTEXT.contains("context7"));
        assert!(DEFAULT_CONTEXT.contains("crabcc"));
        assert!(DEFAULT_CONTEXT.contains("gh issue create"));
    }

    #[test]
    fn context_text_prefers_repo_override() {
        let dir = tempfile::tempdir().unwrap();
        // No override yet -> default.
        assert_eq!(context_text(dir.path()), DEFAULT_CONTEXT);
        // With an override file -> its (trimmed) contents win.
        std::fs::create_dir_all(dir.path().join(".crabcc")).unwrap();
        std::fs::write(
            dir.path().join(".crabcc").join("ctx-inject.md"),
            "  use bun, not npm  \n",
        )
        .unwrap();
        assert_eq!(context_text(dir.path()), "use bun, not npm");
    }
    ...
    fn empty_override_falls_back_to_default() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join(".crabcc")).unwrap();
        std::fs::write(dir.path().join(".crabcc").join("ctx-inject.md"), "   \n").unwrap();
        assert_eq!(context_text(dir.path()), DEFAULT_CONTEXT);
    }
}
