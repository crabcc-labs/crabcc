...
| global allocator (system default)                | `crabcc-cli/src/main.rs` | **re-benched [#648](./docs/PERF-648-agent-shell-and-deps.md): system wins/ties** — system/jemalloc/mimalloc within ~2% (overlapping σ) on cold index *and* refs/graph, two platforms. Shipped build uses system; jemalloc/mimalloc are opt-in knobs. |
...
- **Agent-shell protector**: PreToolUse hook rewrites grep/find → `rg` /
  ...
  **Vanilla vs full flow** — see the auto-generated matrix below.
- **Allocator: system is the default** ([#648 §4](./docs/PERF-648-agent-shell-and-deps.md)) —
  benched fastest (or tied, lowest variance) vs jemalloc + mimalloc on both
  Linux x86 and darwin arm64; the old "+5-12% jemalloc" claim reproduced on
  neither, so the shipped build drops it (both kept as opt-in knobs).

### Token benchmark

<!-- BENCH:START -->
_Auto-generated by `scripts/bench-flow-matrix.sh` — commit `753c244f5789`, 2026-06-04T13:03:19Z, Darwin arm64. Morph stage: off. Tokens = bytes/4, clean `git archive` tree._

**Per agent profile** (vanilla shell vs the full crabcc rewrite flow):

| profile | vanilla | flow | reduction |
|---|---:|---:|---:|
| claude_code  |   136416 |    31442 |    -77% |
| nullclaw     |   101117 |     2799 |    -97% |
| zeroclaw     |   102355 |     2844 |    -97% |

**Per operation:**

| operation | vanilla | flow | reduction |
|---|---:|---:|---:|
| find symbol (grep Store) | 94017 | 2102 | -98% |
| find refs (grep Backend) | 6126 | 194 | -97% |
| list files (find *.rs) | 2212 | 542 | -75% |
| text search (grep pub fn) | 18261 | 12242 | -33% |
| read JSON (cat *.json) | 1010 | 979 | -3% |
| read source (re-read) | 14790 | 122 | -99% |
